...
The team page is built from the 109 people published on venturelab.ca/our-team, grouped as the live site groups them, with 36 headshots and the 23 personal LinkedIn links ventureLAB publishes on its own person pages. Nobody without a published link gets one. The events calendar is built from the events ventureLAB has published, and it emits `Event` markup: the live site presents its calendar as an embedded Google Calendar, which no search or answer engine can read.
